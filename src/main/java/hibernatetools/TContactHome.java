...
/**
 * Home object for domain model class TContact.
 * @see .TContact
 * @author Hibernate Tools
 */
public class TContactHome {

	private static final Log log = LogFactory.getLog(TContactHome.class);
	...
	public void persist(TContact transientInstance) {
		log.debug("persisting TContact instance");
		try {
	...
	public TContact merge(TContact detachedInstance) {
		...
		try {
			TContact result = (TContact) sessionFactory.getCurrentSession().merge(detachedInstance);
	...
	public TContact findById(int id) {
		log.debug("getting TContact instance with id: " + id);
		try {
			TContact instance = (TContact) sessionFactory.getCurrentSession().get("TContact", id);
			if (instance == null) {
	...
	public List findByExample(TContact instance) {
		...
		try {
			List results = sessionFactory.getCurrentSession().createCriteria("TContact").add(Example.create(instance))
					.list();
