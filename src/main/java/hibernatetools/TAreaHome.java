...
/**
 * Home object for domain model class TArea.
 * @see .TArea
 * @author Hibernate Tools
 */
public class TAreaHome {

	private static final Log log = LogFactory.getLog(TAreaHome.class);
	...
	public void persist(TArea transientInstance) {
		log.debug("persisting TArea instance");
		try {
	...
	public TArea merge(TArea detachedInstance) {
		...
		try {
			TArea result = (TArea) sessionFactory.getCurrentSession().merge(detachedInstance);
	...
	public TArea findById(TAreaId id) {
		...
		try {
			TArea instance = (TArea) sessionFactory.getCurrentSession().get("TArea", id);
			if (instance == null) {
	...
	public List findByExample(TArea instance) {
		...
		try {
			List results = sessionFactory.getCurrentSession().createCriteria("TArea").add(Example.create(instance))
					.list();
